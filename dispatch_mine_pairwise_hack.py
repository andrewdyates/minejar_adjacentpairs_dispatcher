...
def main(k=100000, jobname='dispatch_mine', n_nodes=13, n_ppn=4, walltime='6:00:00', work_dir='/fs/lustre/osu6683', minejar_path="/fs/lustre/osu6683/MINE.jar", tab_fname=None, bool_fname=None, dry=False):
  ...
  while offset < n:
    # --------------------
    # HACK
    # do not resubmit job if Results file exists
    mine_results_fname = "%s_offset_%d_%d.tab,adjacentpairs,cv=0.0,B=n^0.6,Results.csv" % (tab_fname.rpartition('.')[0], offset, k)
    if os.path.exists(os.path.join(work_dir, mine_results_fname)):
      print "%s exists. Skipping job batch addition." % mine_results_fname
      offset += k
      n_skipped += 1
      continue
    # --------------------
    ...
    n_jobs += 1
  fp.close()
  print "Generated %d jobs in batch file..." % n_jobs
  print "Skipped %d jobs due to previous results." % n_skipped
